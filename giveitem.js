JsMacros.on("RecvMessage", JavaWrapper.methodToJava(event => {
    ...
    function get_item(ItemInfo, Counts) {
        //扫描物品
        //East sn
        const EastScanPos = [
        ...
        World.getEntities("minecraft:item_frame").forEach(entity => {
            ...
            itemFrameMap.set(pos_to_str(pos), item)
        })
        ...
        function EastscanRow(pos)
        {
            ...
            z = pos.startPos.getZ()
            while(x <= wall) {
                if(itemFrameMap.has(pos_to_str(framefacting(pos, x, y, z)))) {
                    if(itemFrameMap.get(pos_to_str(framefacting(pos, x, y, z))).getName().getString() != "空气")
                        blocks.set(itemFrameMap.get(pos_to_str(framefacting(pos,x,y,z))).getName().getString(), 
                                   {dx:x,dy:y,dz:z,df:(pos.floor),fo:(pos.faceOffset)});
                }
                    ...
                    if(!blocks.has(World.getBlock(framefacting(pos,x,y,z)).getName().getString())) {
                        if(World.getBlock(framefacting(pos,x,y,z)).getName().getString().includes("墙上的失活")) {
                            blocks.set("失活的" + World.getBlock(framefacting(pos,x,y,z)).getName().getString().replace("墙上的失活", ""),
                                       {dx:x,dy:y,dz:z,df:(pos.floor),fo:(pos.faceOffset)})
                        }
                            ...
                            blocks.set(World.getBlock(framefacting(pos,x,y,z)).getName().getString().replace("盆栽",""),
                                       {dx:x,dy:y,dz:z,df:(pos.floor),fo:(pos.faceOffset)})
                        }
                    }
                }
                    ...
                    if(!blocks.has(World.getBlock(x,y,z).getName().getString())) {
                        blocks.set(World.getBlock(x,y,z).getName().getString().replace("盆栽",""), 
                                   {dx:x,dy:y,dz:z,df:(pos.floor),fo:(pos.faceOffset)})
                    }
                }
                x+=facing;
        ...
        function WestscanRow(pos)
        {
            ...
            while(x>=wall) {
                if(itemFrameMap.has(pos_to_str(framefacting(pos, x, y, z)))) {
                    if(itemFrameMap.get(pos_to_str(framefacting(pos,x,y,z))).getName().getString()!="空气")
                        blocks.set(itemFrameMap.get(pos_to_str(framefacting(pos,x,y,z))).getName().getString(),{dx:x,dy:y,dz:z,df:(pos.floor),fo:(pos.faceOffset)});
                }
                else if(World.getBlock(framefacting(pos,x,y,z)).getName().getString()!="空气")
                {
                    if(!blocks.has(World.getBlock(framefacting(pos,x,y,z)).getName().getString()))
        ...
        function getsome()
        {
            ...
            while(slot < 54)
            {
                ...
                else if(Player.openInventory().getSlot(slot).getItemId() != "minecraft:air" 
            ...
            if(hasshulker)
            {
                slotshulkerinfo.sort((x,y) => {
                    if(y.first.getNBT() != null && x.first.getNBT() != null) {
                        return (y.first.getNBT().get("BlockEntityTag").get("Items").asListHelper().length()) 
                                - (x.first.getNBT().get("BlockEntityTag").get("Items").asListHelper().length());
                    }
                    return x.second - y.second
                });
            }
